...
CONFIG = {
    ...
    'host': '<YOUR_DB_HOST>',
    'port': '5432',
    'table_name': 'bj2019_culture_part5_with_response_评价前处理1',
    'new_table_name': 'bj2019_culture_part1_with_response_首都文化分类_abs1',
    'batch_size': 1,  # Batch size
    # Prompt template loaded from external .txt file. The prompt content should match paper Appendix A.
    'system_content_file': '<YOUR_PROMPT_DIR>/物质文化载体ABSA3.txt',
    'additional_column': '古都文化_物质文化载体方面级评价',  # New column name
    'where_condition': '古都文化_评价载体判断 = \'1\'',  # Query condition
    'max_concurrency': 100  # Max concurrency
}
        ...
        try:
            data = {
                ...
                "messages": [
                    ...
                    {"role": "user", "content": f'Social media text: "{row[2]}"\nExtracted cultural carrier: "{row[3]}"'}
                ],
                "temperature": 0.0,
            }
    ...
    try:
        with psycopg2.connect(database=CONFIG['database'], user=CONFIG['user'], password=CONFIG['password'], host=CONFIG['host'], port=CONFIG['port']) as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(f'SELECT COUNT(*) FROM "{CONFIG["table_name"]}" WHERE {CONFIG["where_condition"]};')
                ...
                with Progress(
                    ...
                    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=CONFIG['max_concurrency'])) as session:
                        while True:
                            try:
                                cur.execute(f'SELECT id, tid, cleaned_content, "古都文化_评价载体" FROM "{CONFIG["table_name"]}" WHERE {CONFIG["where_condition"]} ORDER BY tid LIMIT %s OFFSET %s', (CONFIG['batch_size'], offset))
                                rows = cur.fetchall()
                                if not rows:
                                    break
                                ...
                                update_data = [(response_content, tid) for tid, response_content in responses if response_content is not None]
                                if update_data:
                                    cur.executemany(f"UPDATE {CONFIG['new_table_name']} SET {CONFIG['additional_column']} = %s WHERE tid = %s", update_data)
                                    conn.commit()
